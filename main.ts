(() => {
    ...
    const correo = document.getElementById('inpt_correo') as HTMLInputElement;
    const nombre = document.getElementById('inpt_nombre') as HTMLInputElement;
    const telefono = document.getElementById('inpt_telefono') as HTMLInputElement;
    const tbody = document.getElementById('users') as HTMLElement;
    
    const btnEnviarDatos = document.getElementById('btn_enviar') as HTMLButtonElement;
    const btnLimpiarDatos = document.getElementById('btn_limpiar') as HTMLButtonElement;
    const btnActualizarDatos = document.getElementById('btn_actualizar') as HTMLButtonElement;
    
    let indexUser:number;
    let isUpdating:boolean = false;
    
    btnLimpiarDatos?.addEventListener('click', e => {
        let sure:boolean = confirm('¿Estás seguro que quieres limpiar la base de datos?');
        if(sure) {
            localStorage.clear();
            tbody.innerHTML = '';
    ...
    const validForm = () => {
        let emailPattern:RegExp = /[a-zA-Z0-9]+@\w+.\w+/g;
